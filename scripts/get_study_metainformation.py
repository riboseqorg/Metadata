...
def download_GSE_metadata_files(accession: str) -> dict:
    ...
    """
    base_url = (
        f"https://ftp.ncbi.nlm.nih.gov/geo/series/{accession[0:-3]}nnn/{accession}"
    )
...
def get_metainformation(accession: str, database) -> dict:
    ...
    return (
        record
        if type(record) == dict or type(record) == Entrez.Parser.DictionaryElement
        else record
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Script to obtain study metainformation from public repositories given an accession"
    )
