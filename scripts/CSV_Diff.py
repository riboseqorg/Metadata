...
class CSVDiffAnalyzer:
    ...
    def analyze_value_distributions(self, column: str) -> Dict[str, Any]:
        ...
        return {
            'top_values_1': dist1.to_dict(),
            'top_values_2': dist2.to_dict()
        }

    def find_mismatched_values(self, column: str, sample_size: int = 5, output_file: str = None) -> Dict[str, Any]:
        """
        ...
        if output_file and len(mismatches) > 0:
            with open(output_file, 'a') as f:  # 'a' for append mode
                ...
                f.write(f"{'Run':<15} | {'Value in ' + self.csv1_name:<30} | {'Value in ' + self.csv2_name:<30}\n")
                f.write("-" * 80 + "\n")
                
                for idx, row in mismatches.iterrows():
                    f.write(f"{row['Run']:<15} | {str(row[f'{column}_1']):<30} | {str(row[f'{column}_2']):<30}\n")
    ...
    def generate_difference_report(self, output_file: str):
        ...
        with open(output_file, 'w') as f:
            ...
            f.write(f"Columns unique to {self.csv1_name}: {len(cols['unique_to_first'])}\n")
            f.write(f"{cols['unique_to_first']}\n\n")
            f.write(f"Columns unique to {self.csv2_name}: {len(cols['unique_to_second'])}\n")
            ...
            f.write(f"Percentage identical: {identical_rows['identical_percentage']:.2f}%\n\n")
            
            # Value differences in key columns
            f.write("=== Value Differences in Key Columns ===\n")
            key_columns = ['CELL_LINE', 'TISSUE', 'INHIBITOR', 'CONDITION', 'LIBRARYTYPE', 
            ...
            for col in key_columns:
                if col in identical['different']:
                    self.find_mismatched_values(col, sample_size=None, output_file=output_file)
    ...
    def interactive_analysis(self):
            ...
            while True:
                ...
                if choice == '1':
                    ...
                    print(tabulate([
                        [self.csv1_name, results['nan_count_1'], f"{results['nan_percent_1']:.2f}%"],
                        [self.csv2_name, results['nan_count_2'], f"{results['nan_percent_2']:.2f}%"]
...
def main():
    parser = argparse.ArgumentParser(description='Interactive tool to analyze differences between two CSV files.')
