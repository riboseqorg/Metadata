...
import numpy as np
import requests
from get_study_metainformation import (
    get_metainformation,
    xmlData_to_dict,
    download_GSE_metadata_files,
)
...
def get_metainformation_dict(df: pd.DataFrame) -> dict:
    ...
    record["ScientificName"] = "; ".join(
        list(df["ScientificName"].astype(str).unique())
    )
    ...
    record["Release_Date"] = "/".join(
        [str(df["YEAR"].unique()[0]), str(df["MONTH"].unique()[0]), "00", "00:00"]
    )
    ...
    if list(df["LIBRARYTYPE"].unique()) == ["nan"]:
        record["seq_types"] = (
            ";".join(list(df["LIBRARYTYPE"].unique()))
            .replace("RFP", "Ribo-Seq")
            .replace("RNA", "RNA-Seq")
        )
    ...
    if list(df["AUTHOR"].unique()) != ["nan"] or list(df["AUTHOR"].unique()) != [
        "Makar"
    ]:
    ...
    if record["PMID"] != "":
        ...
        if "Unknown" in record["Name"] and "Unknown" not in record["Authors"]:
            record[
                "Name"
            ] = f"{record['Authors']} et al. {record['Date_published'].split(' ')[0]}"
